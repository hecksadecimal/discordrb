...
module Discordrb::Webhooks
  ...
  class Client
    ...
    def execute(builder = nil, wait = false, thread_id = nil, components = nil)
    ...
    def post_json(builder, components, wait, thread_id)
      ...
      RestClient.post(@url + (wait ? '?wait=true' : '') + (thread_id ? (wait ? '&' : '?') + "=#{thread_id}" : ''), data.to_json, content_type: :json)
    end

    def post_multipart(builder, components, wait, thread_id)
      data = builder.to_multipart_hash.merge({ components: components.to_a })
      RestClient.post(@url + (wait ? '?wait=true' : '') + (thread_id ? (wait ? '&' : '?') + "=#{thread_id}" : ''), data)
    end
